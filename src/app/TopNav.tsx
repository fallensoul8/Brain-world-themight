...
export type AppTab = 'town' | 'chat' | 'simulation' | 'factions' | 'groupchat' | 'graph'
  ...
  return (
    <nav className="relative z-40 flex items-center h-12 px-4 bg-bg-canvas border-b border-border-subtle shrink-0 select-none">
      ...
      <div className={cn(
        ...
        {(['town', 'chat', 'simulation', 'factions', 'groupchat', 'graph'] as const).map((tab) => {
          const label = tab === 'town' ? 'Town' : tab === 'chat' ? 'Chat' : tab === 'simulation' ? '🧠 AI' : tab === 'factions' ? '🏰' : tab === 'groupchat' ? '💬' : '🔄'
          return (
            <button
              key={tab}
